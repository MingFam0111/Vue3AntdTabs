        ...
        <a-tabs v-model:activeKey="activeKeyTab">
            ...
            <template #rightExtra>
                ...
                <div v-if="activeKeyTab === '1'">
                    <a-button shape="circle" :disabled="activeKeyTab === '1' && activeKeyCollapse.length < 1"
                        :icon="h(UpOutlined)" @click="() => { handleCloseCollapseAllInTab() }" />
                    <a-button shape="circle" :disabled="activeKeyTab === '1' && activeKeyCollapse.length === 3"
                        :icon="h(DownOutlined)" @click="() => { handleOpenCollapseAllInTab() }" />
                </div>

                <!-- Tab 2 -->
                <div v-if="activeKeyTab === '2'">
                    <a-button shape="circle" :disabled="activeKeyTab === '2' && activeKeyCollapse2.length < 1"
                        :icon="h(UpOutlined)" @click="() => { handleCloseCollapseAllInTab() }" />
                    <a-button shape="circle" :disabled="activeKeyTab === '2' && activeKeyCollapse2.length === 3"
                        :icon="h(DownOutlined)" @click="() => { handleOpenCollapseAllInTab() }" />
                </div>

                <!-- Tab 3 -->
                <div v-if="activeKeyTab === '3'">
                    <a-button shape="circle" :disabled="activeKeyTab === '3' && activeKeyCollapse3.length < 1"
                        :icon="h(UpOutlined)" @click="() => { handleCloseCollapseAllInTab() }" />
                    <a-button shape="circle" :disabled="activeKeyTab === '3' && activeKeyCollapse3.length === 3"
                        :icon="h(DownOutlined)" @click="() => { handleOpenCollapseAllInTab() }" />
                </div>
            </template>
        </a-tabs>
